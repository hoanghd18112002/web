...
import { CTKho } from 'src/app/models/ctkho.model';
...
@Component({
  ...
  styleUrls: ['./qlkho.component.css']
})
export class QlkhoComponent {
  ListKho: Kho[] = [];
  ListCTKho: CTKho[] = [];
  ...
  getall(){
    ...
    this.khoService.getall(obj).subscribe(res => {
  ...
  add() {
    ...
    this.khoService.create(kho).subscribe(res => {
      this.getall();
      swal.fire({
        icon: 'success',
        title: res.message,
  ...
  update() {
    if (this.selectedRow) {
      ...
      this.khoService.update(kho).subscribe(res => {
        this.getall();
        swal.fire({
          icon: 'success',
          title: res.message,
  ...
  delete() {
    if (this.selectedRow) {
      ...
      this.khoService.detele(id).subscribe(res => {
        this.getall();
        swal.fire({
          icon: 'success',
          title: res.message,
  ...
  selectedRow: Kho | null = null;

  onRowClick(kho: Kho) {
    this.selectedRow = kho;

    this.ten = this.selectedRow.ten;
    this.diaChi = this.selectedRow.diaChi;
    this.trangThai = this.selectedRow.trangThai;

    this.khoService.getbykho(this.selectedRow.id).subscribe(res => {
      this.ListCTKho = res.data;

      for (const item of this.ListCTKho) {
