...
export class LoaiSanPhamService {
  private apiUrl = 'http://localhost:5226/api/loaisanpham';
  ...
  getall(loaisanpham: object): Observable<any> {
    const headers = new HttpHeaders().set('Authorization', 'Bearer ' + _user.token);
    return this.http.post<any>(`${this.apiUrl}/get-all`, loaisanpham, { headers });
  }
