...
export class PhuongThucService {
  private apiUrl = 'http://localhost:5226/api/phuongthucthanhtoan';
  ...
  getall(phuongthuc: object): Observable<any> {
    const headers = new HttpHeaders().set('Authorization', 'Bearer ' + _user.token);
    return this.http.post<any>(`${this.apiUrl}/get-all`, phuongthuc, { headers });
  }
