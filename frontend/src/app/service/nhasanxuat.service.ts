...
export class NhaSanXuatService {
  private apiUrl = 'http://localhost:5226/api/nhasanxuat';
  ...
  getall(nhasanxuat: object): Observable<any> {
    const headers = new HttpHeaders().set('Authorization', 'Bearer ' + _user.token);
    return this.http.post<any>(`${this.apiUrl}/get-all`, nhasanxuat, { headers });
  }
