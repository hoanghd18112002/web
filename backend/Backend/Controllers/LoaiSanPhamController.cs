...
namespace Backend.Controllers
{
    ...
    {
        private ILoaiSanPhamBLL _bll;
        private string _path;
        public LoaiSanPhamController(ILoaiSanPhamBLL bll, IConfiguration configuration)
        {
            _bll = bll;
            _path = configuration["AppSettings:PATH_LOAISANPHAM"];
        }
        ...
        [Route("create")]
        [HttpPost]
        public IActionResult Create([FromForm] LoaiSanPhamModel model)
        {
            try
            {
                ...
                {
                    Ten = model.Ten,
                    BieuTuong = imagePath, // Đường dẫn tương đối của ảnh
        ...
        [Route("update")]
        [HttpPut]
        public IActionResult Update([FromForm] LoaiSanPhamModel model)
        {
            try
            {
                ...
                if (Request.Form.Files.Count > 0)
                {
                    ...
                    if (!string.IsNullOrEmpty(Model.BieuTuong))
                    {
                        string oldFilePath = Path.Combine(_path, Model.BieuTuong);
                    ...
                    model.BieuTuong = uniqueFileName;
                }
        ...
        public IActionResult Delete(int id)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(model.BieuTuong))
                {
                    string filePath = Path.Combine(_path, model.BieuTuong);
